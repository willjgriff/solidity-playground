...
### ByzantiumUpgradable
Set of contracts enabling upgradability post the Byzantium HF. Based largely on the structure of the EtherRouter but which uses the new Assembly opcodes `returndatacopy` and `returndatasize` to remove the need for the EtherRouter's Resolver.
