...
var balances = () => {
	...
	balance(2)
	balance(3)
	token.balanceOf(FeeVote.address)
		.then(balance => console.log("FeeVote balance: " + balance))
}
...
var castVote = (account, vote) => feeVote.getSealedVote(web3.eth.accounts[account], vote, salt)
	.then(sealedVote => feeVote.castVote(sealedVote, { from: web3.eth.accounts[account] })
		.then(tx => console.log("Sealed vote cast acc" + account + ": " + sealedVote)))

var revealVote = (account, vote) => feeVote.revealVote(vote, salt, { from: web3.eth.accounts[account] })
	.then(tx => console.log("Vote revealed for acc" + account))

var claimReward = (account) => feeVote.claimReward({ from: web3.eth.accounts[account] })
...
// var debugEvent2 = feeVote.Debug2();
